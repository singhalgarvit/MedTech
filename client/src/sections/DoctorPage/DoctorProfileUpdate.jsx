...
function DoctorProfileUpdate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.clinicLocation.trim() !== "") formData.append("clinicLocation", form.clinicLocation.trim());
      ...
      setProfileImageFile(null);
      const updated = await getMyDoctorProfile();
      setProfile(updated);
      setForm({
        name: updated.name ?? "",
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-6">Edit only the fields you want to change. Existing details are shown below; all fields are optional.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Profile picture <span className="text-gray-400 font-normal">(optional — choose a file only to change)</span></label>
        ...
        <div>
          <label htmlFor="clinicLocation" className="block text-sm font-medium text-gray-700 mb-1">
            Clinic location <span className="text-gray-400 font-normal">(optional)</span>
          </label>
          <input
            id="clinicLocation"
            name="clinicLocation"
            ...
            onChange={handleChange}
            placeholder={profile?.clinicLocation ? undefined : "Clinic address or area"}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            placeholder={profile?.experience != null ? undefined : "0"}
            className="w-full rounded-lg border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          />
        </div>

...
function PatientPreviousAppointments() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const previous = (Array.isArray(data) ? data : []).filter(
            (apt) => ["completed", "cancelled", "rejected", "no_show"].includes(apt.status)
          );
  ...
  if (appointments.length === 0) return <p className="text-gray-500">No previous appointments found.</p>;
            ...
            {appointments.map((apt) => (
              <tr key={apt._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${apt.status === "completed"
